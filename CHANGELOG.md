...
- Switched npm publish workflow to use Trusted Publishing (see [https://docs.npmjs.com/trusted-publishers](https://docs.npmjs.com/trusted-publishers)).

## [1.0.21] - 2026-01-27

### Fixed

- The page resource loader watcher routine (started via `setInterval()`) was not always clean up and could keep running even after main() has exited.
...
- A new commandline option was introduced: `--page-error-text-pattern`. Its purpose is to check the final stage of the page (right before we render a PDF from it) for the occurrence of certain strings and cause an error (and a retry) if the string was found. The VolvoCars manuals use Next.js (and React) and sometimes they unexpectedly crash with a "Application error: a client-side exception has occurred while loading www.volvocars.com (see the browser console for more information)" error message. The default values of the new cmdline parameter are meant to detect such errors and simply cause a page rendering retry.
