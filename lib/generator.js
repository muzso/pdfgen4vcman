  ...
  retObj.browserPage.on("response", async (response) => {
    ...
    if ([ "http:", "https:" ].includes(parsedResponseUrl.protocol)) {
      ...
      if (response.ok()) {
        ...
        if (options.resourceHttpErrorDomainSuffixes.some((x) => hostname.endsWith(x)) && options.resourceHttpErrors.includes(responseStatusCode)) {
      ...
      if (value === pageLoadErrorSignal) {
        ...
        if (value) {
          ...
          try {
            ...
            if (options.pageHttpErrors.includes(httpStatus)) {
