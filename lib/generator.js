...
async function generatePagePdf(pageUrl, pdfFilePath, browserPage, options, isToCPage) {
  ...
  try {
    ...
    // removing a couple of most certainly unwanted sections
    // e.g. "Customised support ... sign-in", header (with site links), footer
    const failedSelectors = await browserPage.evaluate(() => {
      const result = [];
      [
        "body div#site-nav-embed", // header
        "body div#site-footer-embed", // footer
    ...
    failedSelectors.forEach((selector) => {
      logger.warn(`generatePagePdf(): could not find any element for the "${selector}" CSS selector (this might not be normal)`);
    });
    ...
    if (options.links !== true) {
      const failedSelectors2 = await browserPage.evaluate(() => {
        [
    ...
    await browserPage.evaluate(() => {
      // replacing anchor tags with span tags since we don't need hyperlinks
      // in a PDF
    ...
    // or an iframe. Here we remove all iframes that have an "src" since normally
    // VolvoCars doesn't use any iframes and this "ethnio" iframe was loaded from
    // a 3rd party URL (i.e. non-empty "src" property).
