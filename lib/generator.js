    ...
    if ([ "http:", "https:" ].includes(parsedResponseUrl.protocol)) {
      ...
      if (!options.resourceHttpError.includes(responseStatusCode)) {
        ...
        if (options.resourceHttpErrorDomainSuffix.some((x) => hostname.endsWith(x))) {
          ...
          logger.verbose(`page.on("response"): response for ${responseUrl} was not OK, but domain suffix is on the exception list, status: ${responseStatusCode} (fromCache: ${responseFromCache}, responseTime: ${responseTime} ms)`);
        }
      }
    }
  });
...
async function generatePagePdf(pageUrl, pdfFilePath, browserPage, options, isToCPage) {
  ...
  try {
    ...
    //       the state of `pageLoadError` and if a URL loading error is detected,
    ...
    if (options.leniency <= 11 && pageLoadError) {
    ...
    if (options.leniency <= 10 && pageLoadError) {
    ...
    if (options.leniency <= 9 && pageLoadError) {
    ...
    if (options.leniency <= 8 && pageLoadError) {
    ...
    if (!isToCPage && options.links !== true) {
      await browserPage.evaluate(() => {
        [
          "body div[class*=\"ArticlePageLayout_relatedArticles\"]",
          "body div:has(> div[class*=\"TitleCardList_container\"])"
        ].forEach((selector) => {
          document.querySelectorAll(selector).forEach((element) => {
            element.remove();
          });
    ...
    if (rejectAllCookiesButtonHandle) {
      ...
      // look for the "Accept" button of the cookie consent overlay and click it if it's there and visible
      const acceptCookiesButtonHandle = await browserPage.$("body #onetrust-accept-btn-handler");
      if (acceptCookiesButtonHandle) {
        if (await acceptCookiesButtonHandle.isVisible()) {
          await acceptCookiesButtonHandle.click();
          logger.verbose("generatePagePdf(): clicked the \"Accept\" button on the cookie consent overlay");
          // wait for the button to disappear
          const acceptCookiesButtonHandle2 = await browserPage.waitForSelector("body #onetrust-accept-btn-handler", { hidden: true });
          if (acceptCookiesButtonHandle2) acceptCookiesButtonHandle2.dispose();
        }
        acceptCookiesButtonHandle.dispose();
      } else {
        logger.verbose("generatePagePdf(): no button was found on the cookie consent overlay");
      }
    }
    ...
    const failedSelectors = await browserPage.evaluate((isToCPage) => {
      const result = [];
      [
      ...
      if (!isToCPage) {
        ...
        const titleDivSelector = "body div > div:has(> h1[class*=\"ArticleTitle_title\"])";
        return new Promise((resolve) => {
          let titleDivSelectorCheckCount = 50;
          const intervalID = setInterval(() => {
            const titleDivElements = document.querySelectorAll(titleDivSelector);
            if (titleDivElements.length == 1) {
              const titleDivElement = titleDivElements[0];
              if (titleDivElement.previousElementSibling) {
                if (
                  titleDivElement.previousElementSibling.tagName
                  && titleDivElement.previousElementSibling.tagName.toUpperCase() == "DIV"
                ) {
                  const buttonCount = titleDivElement.previousElementSibling.querySelectorAll("button").length;
                  if (buttonCount == 1) {
                    titleDivElement.previousElementSibling.remove();
                    clearInterval(intervalID);
                    resolve(result);
                  } else {
                    if (buttonCount == 0) {
                      result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / previousElementSibling doesn't have a button");
                    } else {
                      result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / previousElementSibling has too many buttons: " + buttonCount);
                    }
                  }
                } else {
                  result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / previousElementSibling not a DIV");
                }
              } else {
                result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / no previousElementSibling");
              }
            } else {
              if (titleDivElements.length == 0) {
                result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / not found");
              } else {
                result.push(titleDivSelector + " (" + titleDivSelectorCheckCount + ") / too many: " + titleDivElements.length);
              }
            }
            titleDivSelectorCheckCount--;
            if (titleDivSelectorCheckCount == 0) {
              clearInterval(intervalID);
              resolve(result);
            }
          }, 100);
    ...
    failedSelectors.forEach((selector) => {
      logger.verbose(`generatePagePdf(): could not find any element for the "${selector}" CSS selector (this might not be normal)`);
    });
