      ...
      if (!options.resourceHttpError.includes(responseStatusCode)) {
        ...
        if (options.resourceHttpErrorDomainSuffix.some((x) => hostname.endsWith(x))) {
          if (options.resourceHttpErrorUrlException.some((x) => x.test(responseUrl))) {
            logger.verbose(`page.on("response"): response for ${responseUrl} was not OK, but URL is on the regexp exception list, status: ${responseStatusCode} (fromCache: ${responseFromCache})`);
          } else {
            logger.error(`page.on("response"): error for ${responseUrl}, status: ${responseStatusCode} (fromCache: ${responseFromCache})`);
            pageLoadError = true;
          }
        } else {
          logger.verbose(`page.on("response"): response for ${responseUrl} was not OK, but domain suffix is on the exception list, status: ${responseStatusCode} (fromCache: ${responseFromCache})`);
        }
      }
    }
  });
        ...
        if (titleDivElement.previousElementSibling) {
          if (
            ...
            if (titleDivElement.previousElementSibling.querySelector("button") !== null) {
              ...
              result.push(titleDivSelector + " / previousElementSibling doesn't have a button");
            }
          } else {
            result.push(titleDivSelector + " / previousElementSibling not a DIV");
          }
        } else {
          result.push(titleDivSelector + " / no previousElementSibling");
        }
      } else {
        result.push(titleDivSelector);
      }
