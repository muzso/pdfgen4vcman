  ...
  try {
    ...
    if (isToCPage) {
      ...
      await browserPage.evaluate((titleCaptionStr) => {
        ...
        headings.forEach((element) => {
            element.style.padding = '0.25em 0';
            element.style.margin = '1em 0 0';
        });
        // apply a more compact look to the list
        [
          "body section#ownersmanual > ul li",
          "body section#ownersmanual > ul li div",
          "body section#ownersmanual > ul li a"
        ].forEach((selector) => {
          const items = document.querySelectorAll(selector);
          items.forEach((element) => {
            element.style.padding = '0.1em 0';
          })
        });
    ...
    if (options.links !== true) {
      await browserPage.evaluate(() => {
        [
          "body main div[class*=\"ArticlePageLayout_relatedArticles\"]",
          "body main div:has(> div[class*=\"TitleCardList_container\"])"
        ].forEach((selector) => {
    ...
    const failedSelectors = await browserPage.evaluate(() => {
      ...
      [
        ...
        "body nav + script + div > div:has(p + button)", // sign-in (doesn't appear on ToC page)
        "body nav + script + div > div:has(> hr)", // horizontal ruler at the bottom of main page content
        "body #ethnio-campaign-theme", // Volvo introduced a campaign (in Aug 2024) to collect feedback on their support website
          // and offered money for participation.
        "body #quiqWebChat", // chat popup button
        "body nav + script + div > div > div:has(h2 + div[class^='TitleCard'])", // "More in this topic" links (not on all pages)
        "body nav + script + div > div > div > aside:has(h2 + div)", // related articles (not on all pages)
        "body [aria-label=Breadcrumb]", // breadcrumbs (must me last rule for "nav + script + div" selectors to work!)
      ].forEach((selector) => {
        const elements = document.querySelectorAll(selector);
        if (elements.length > 0) {
          elements.forEach((element) => {
